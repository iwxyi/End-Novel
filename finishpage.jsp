...
<html>
	...
	<body class="mdui-appbar-with-toolbar mdui-drawer-body-left">
		...
		<div class="mdui-tab mdui-tab-full-width" mdui-tab>
			...
			<a href="finishpage.jsp" class="mdui-ripple mdui-tab-active">完本</a>
		...
		<%
			String source = "", line = "", url = "https://www.qidian.com/finish";
		...
		<%
			...
			String fiPart = source.substring(startPos, endPos);
		%>
		...
		<div class="mdui-card" style="left: 5%; width: 90%; margin-top: 1%;">
			<%
				...
				String regEx ="<li data-rid=\"\\d+\">.+?<a.+?data-bid=\"(\\d+)\".+?<img src=\"(.+?)\".+?>.+?data-bid=\"\\d+\">(.+?)</a>.+?target=\"_blank\">(.+?)</a>.+?<p class=\"intro\">\\s*(.+?)</p>";
				Pattern p = Pattern.compile(regEx, Pattern.CASE_INSENSITIVE);
				Matcher m = p.matcher(fiPart);
				while (m.find())
				{
					bookID = m.group(1);
				...
				String maxPages = "1";
				int maxPage = 0;
				regEx ="data-pagemax=\"(\\d+)\"";
				p = Pattern.compile(regEx, Pattern.CASE_INSENSITIVE);
				m = p.matcher(source);
				if (m.find())
					maxPages = m.group(1);
				out.print("<li class=\"mdui-list-item mdui-ripple\">1 / "+maxPages+"　<font color='blue'>下一页</font></li>");
			%>
		</div>
