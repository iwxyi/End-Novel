...
import mybean.data.fbzp_Bean; //引入例子2中的Javabean模型
import java.io.*;
import java.sql.*;
import javax.servlet.*;
import javax.servlet.http.*;
public class fbzp_Servlet extends HttpServlet{
   public void init(ServletConfig config) throws ServletException{
      super.init(config);
   }
   public void doPost(HttpServletRequest request,HttpServletResponse response)
               throws ServletException,IOException{
      fbzp_Bean resultBean=null;
      try{  resultBean=(fbzp_Bean)request.getAttribute("resultBean");
            if(resultBean==null){
                resultBean=new fbzp_Bean(); //创建Javabean对象
